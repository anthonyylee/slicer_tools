...
<br/><br/>
Icon source <a href="https://icons8.com/icons/set/silhouette">acknowledgement</a>.
"""
...
class SilhouetteWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def setup(self) -> None:
        ...
        self.ui.outputModelSelector.addAttribute("vtkMRMLModelNode", self.moduleName + ".Role", "OutputSilhouette")
    ...
    def onApplyButton(self) -> None:
        ...
        segmentationNode = self.ui.outputSegmentSelector.currentNode()
        if segmentationNode is None:
            self.showStatusMessage("Segmentation node is None.")
            return
        segmentID = self.ui.outputSegmentSelector.currentSegmentID()
        if segmentID is None or segmentID == "" :
            self.showStatusMessage("Segment ID is None or empty.")
            return
        modelNode = self.ui.outputModelSelector.currentNode()
        if modelNode is None:
            modelNode = self.ui.outputModelSelector.addNode("vtkMRMLModelNode")
        if not modelNode.GetDisplayNode(): # Added in place.
            modelNode.CreateDefaultDisplayNodes()
        self.logic.process(segmentationNode, segmentID, modelNode)
        self.ui.outputModelSelector.setCurrentNode(modelNode)
#
# SilhouetteLogic
#
...
class SilhouetteLogic(ScriptedLoadableModuleLogic):
    ...
    def process(self, segmentationNode, segmentID, outputModelNode) -> None:
        ...
        # Restrict to first 3D view where it is updated.
        views3D = slicer.mrmlScene.GetNodesByClass("vtkMRMLViewNode")
        first3DViewID = views3D.GetItemAsObject(0).GetID()
        outputModelNode.GetDisplayNode().SetViewNodeIDs([first3DViewID])
