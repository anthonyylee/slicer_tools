...
<br/><br/>
Icon source <a href="https://icons8.com/icons/set/body">acknowledgement</a>.
"""
...
class BodyIsolationWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self._keepSegmentationAction = None
        self._removeSourceVolumeAction = None
    ...
    def setup(self) -> None:
        ...
        self._keepSegmentationAction = qt.QAction("Keep segmentation")
        self._keepSegmentationAction.setCheckable(True)
        self._keepSegmentationAction.checked = True
        self.ui.applyButton.menu().addAction(self._keepSegmentationAction)
        self._removeSourceVolumeAction = qt.QAction("Remove source volume")
        self._removeSourceVolumeAction.setCheckable(True)
        self._removeSourceVolumeAction.checked = False
        self.ui.applyButton.menu().addAction(self._removeSourceVolumeAction)
    ...
    def onApplyButton(self) -> None:
        ...
        with slicer.util.tryWithErrorDisplay("Failed to compute results.", waitCursor=True):
            ...
            splitVolumeNode = self.logic.process(self._parameterNode.inputVolume,
                                                 self._keepSegmentationAction.checked)
            if self._removeSourceVolumeAction.checked:
                slicer.mrmlScene.RemoveNode(self._parameterNode.inputVolume)
            self._parameterNode.inputVolume = splitVolumeNode
...
class BodyIsolationLogic(ScriptedLoadableModuleLogic):
    ...
    def process(self, volumeNode, keepSegmentation = False) -> None:
        ...
        # Preferred default; is non-destructive for further work, we may not check it.
        widgetEditor.mrmlSegmentEditorNode().SetOverwriteMode(widgetEditor.mrmlSegmentEditorNode().OverwriteNone)
        ...
        inputVolumeName = slicer.mrmlScene.GenerateUniqueName(volumeNode.GetName())
